...
//définition d'une méthode pour valider une tâche à notre array, méthode appelée suite à la réception d'un event "valid-task" du composant "ListTasks" auquel est annexée la value 'idtask'
  ...
  //alert(idtask)
   ...
   const updateFireBase = () => {
      ...
      const api = fetch("https://test-bili-default-rtdb.europe-west1.firebasedatabase.app/tasks.json",{
        method: "put",
  ...
  const init = () => {
    ...
    fetch("https://test-bili-default-rtdb.europe-west1.firebasedatabase.app/tasks.json")
        .then((response) => response.json())
